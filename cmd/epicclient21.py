...
class EpicClient(object):
	...
	if statuscode == 200:
            self._debugMsg(str(method), "Request completed. No error, operation successful " +str(statuscode))   	
	# HTTP-request POST
	elif statuscode == 201:
	    self._debugMsg(str(method), "Successful creation of a resource " +str(statuscode))
	# HTTP-request POST, PUT, DELETE
	elif statuscode == 202:
	    self._debugMsg(str(method), "The request was received " +str(statuscode))
        # HTTP-request POST, PUT, DELETE
	elif statuscode == 204:
	    self._debugMsg(str(method), "The request was processed successfully, but no response body is needed " +str(statuscode))	
	elif statuscode == 304:
	    self._debugMsg(str(method), "Resource has not been modified " +str(statuscode))
	    output = False
	# HTTP-request GET, PUT, DELETE
	elif statuscode == 400:
	...
	# HTTP-request GET, PUT, DELETE
	    ...
	    self._debugMsg(str(method), "Action requires user authentication " +str(statuscode))    
	    output = False
	elif statuscode == 404:
	...
	# HTTP-request PUT, DELETE
	    ...
	    self._debugMsg(str(method), "Internal Server Error " +str(statuscode))	
	    output = None
	elif statuscode == 501:
	    self._debugMsg(str(method), "Requested HTTP operation not supported " +str(statuscode))
	    output = None
	elif statuscode == 503:
...
#            return False
        ...
        response, _ = self.http.request(uri, method='PUT', headers=hdrs, body=handle_json)
	...
	uri = self._getURI(prefix,'','',suffix)
        ...
        if loc10320 is None:
            ...
            if not lt.checkInclusion(value):
                ...
                self._debugMsg('removeLocationFromHandle',"the location " + value + " cannot be removed")
            return False
...
class Credentials(object):
    ...
    def parse(self):
	    ...
	    #self._debugMsg("irods", "Function getting credential store in iRODS is in testing ...")
...
#            conn, _ = irods.rcConnect(myEnv.getRodsHost(), myEnv.getRodsPort(), myEnv.getRodsUserName(), myEnv.getRodsZone())
#            if self.debug:
...
def test(args):
    ...
    print ("Deleting EMAIL parameter from " + credentials.prefix +
           "/TEST_CR1 (should be True)")
    #fail += test_result(ec.modifyHandle(credentials.prefix + "/TEST_CR1","EMAIL", None), True)
    # -> this line causes TypeError: cannot concatenate 'str' and 'NoneType' objects
    # -> so, None should be changed into "", means parameter for 'EMAIL' is empty 		
    fail += test_result(ec.modifyHandle(credentials.prefix + "/TEST_CR1","EMAIL", ""), True)
    ...
    # FIXME this test is wrong; it doesn't result in "None" -> Explain in above lines.
    fail += test_result(ec.getValueFromHandle(credentials.prefix + "/TEST_CR1", "EMAIL"), None)

    print
    print ("Updating handle info with a new 10320/loc type location "
           "846/157c344a-0179-11e2-9511-00215ec779a8")
    print "(should be False)"
    fail += test_result(ec.updateHandleWithLocation(credentials.prefix + "/TEST_CR1", "846/157c344a-0179-11e2-9511-00215ec779a8"), False)
